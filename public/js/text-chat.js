...
function registerEventHandler() {
  ...
  socket.on('connect', () => {
    ...
    socket.on('other-join', (otherSocketId, room) => {
      ...
      dc = initDataChannel(pc.createDataChannel('text-chat'));
...
function createPeerConnection() {
  ...
  if (!pc) {
    ...
    // 接收来自远端的数据通道数据
    pc.ondatachannel = (e) => {
      dc = dc || initDataChannel(e.channel);
    }
  }
...
const chatEl = document.querySelector('textarea#chat');
const sendTextEl = document.querySelector('textarea#sendtext');
...
function initDataChannel(dataChannel) {
  ...
  dataChannel.close = handleDataChannelStateChange;
  return dataChannel;
}

/**
 * 接收数据通道的消息
 * @param {MessageEvent<any>} e message event
 */
function receiveMessage(e) {
  // 获取远端传输过来的数据
  const message = e.data;
  if (message) {
    chatEl.value += `->${message}\r\n`;
  } else {
